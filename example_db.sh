...
curl -X POST http://$db_user:$db_pass@$db_host:$db_port/husharu_db -H 'Content-type: application/json' -d "{\"level\": \"comment\", \"parent_id\": \"\", \"product_id\": \"$sobha\", \"posted_by\": \"Shady Joe\", \"created_at\": \"1322550106\", \"comment\": \"This project is yet to start.\" }"

# create another comment
curl -X POST http://$db_user:$db_pass@$db_host:$db_port/husharu_db -H 'Content-type: application/json' -d "{\"level\": \"comment\", \"parent_id\": \"\", \"product_id\": \"$sobha\", \"posted_by\": \"Shady Joe\", \"created_at\": \"1322551106\", \"comment\": \"This is another comment about Sobha forest view.\" }"

bsnl=`curl -X POST http://$db_user:$db_pass@$db_host:$db_port/husharu_db -H "Content-type: application/json" -d '{"level": "product", "display_name": "BSNL Landline"}' | grep -Po '"id":.*?",' | perl -pe 's/"id"://; s/^"//; s/",$//'`
curl -X POST http://$db_user:$db_pass@$db_host:$db_port/husharu_db -H 'Content-type: application/json' -d "{\"level\": \"comment\", \"parent_id\": \"\", \"product_id\": \"$bsnl\", \"posted_by\": \"Shady Joe\", \"created_at\": \"1322531106\", \"comment\": \"Do the BSNL guys actually repair anything?\" }"
curl -X POST http://$db_user:$db_pass@$db_host:$db_port/husharu_db -H 'Content-type: application/json' -d "{\"level\": \"comment\", \"parent_id\": \"\", \"product_id\": \"$bsnl\", \"posted_by\": \"Shady Joe\", \"created_at\": \"1312531106\", \"comment\": \"BSNL guys fixed my modem yesterday.  It is not working again!\" }"
